import { useState, useEffect } from "react";
import { generarNumeroAleatorio } from "../services/num_aleatorio";

export function PokeCard() {
    const [pokemon, setPokemon] = useState(null);
    const [isDisabled, setIsDisabled] = useState(false);
    const [num, setNum] = useState(Math.floor(Math.random() * 1025) + 1); // prueba a usar tambien la funcion de generar numero.
    console.log(num);

    useEffect(() => {
        setIsDisabled(false);
        // recuperamos la api
        fetch(`https://pokeapi.co/api/v2/pokemon/${num}/`)
            .then((res) => res.json())
            .then((response) => {
                console.log(response);
                // setPokemon(response)
                setTimeout(() => {
                    setIsDisabled(true);
                    setPokemon(response);
                }, 2000);
            }); // pueba a añadir logina de gestion de errores con un catch. tambio te recomiendo que aisles la peticion en un funcion asyncrona.
    }, [num]);

    const handleClick = () => {
        generarNumeroAleatorio().then((newNum) => setNum(newNum)); // el then se utilizar en funciones que pueden dar error
        /* sugerencia:
        setNum(generarNumeroAleatorio)
        */
    };

    console.log(pokemon?.base_experience);
    console.log(pokemon?.stats[0].base_stat);

    return (
        //! tenemos que verificar que pokemon existe porque si no da error
        //! NO PUEDE SER EN EL DIV PRINCIPAL
        <>
            {pokemon && (
                <article className="rounded-lg text-xxs w-52 h-64 items-center flex flex-col gap-3 bg-white font-bold">
                    <div className="relative w-full flex items-center justify-around z-10">
                        <img
                            className="absolute rounded-t-lg w-full h-16 top-0"
                            src="../src/assets/bg-pattern-card.svg"
                            alt="tarjeta-color"
                        />
                        <div className="relative mt-3">
                            <div className="w-24 h-24 rounded-full bg-white mb-3 flex items-center justify-center overflow-hidden">
                                <img
                                    src={
                                        pokemon?.sprites.other.showdown[
                                            "front_default"
                                        ]
                                    }
                                    alt="gifPokemon"
                                />
                            </div>
                            <div className="flex flex-col justify-center items-center">
                                <h1>
                                    {pokemon?.name}
                                    <span className="text-darkGray">
                                        {" "}
                                        {pokemon?.stats[0].base_stat}
                                        {pokemon?.stats[0].stat.name}
                                    </span>
                                </h1>
                                <p className="text-darkGray">
                                    {pokemon?.base_experience} exp
                                </p>
                            </div>
                        </div>
                    </div>
                    <div className="relative border-t border-darkGray pt-5 flex items-center justify-center">
                        <div className="flex items-center justify-center flex-col gap-1 mx-3">
                            <h3>{pokemon?.stats[1].base_stat}</h3>
                            <p className="text-darkGray">Ataque</p>
                        </div>
                        <div className="flex items-center justify-center flex-col gap-1 mx-3">
                            <h3>{pokemon?.stats[3].base_stat}</h3>
                            <p className="text-darkGray">Ataque especial</p>
                        </div>
                        <div className="flex items-center justify-center flex-col gap-1 mx-3">
                            <h3>{pokemon?.stats[2].base_stat}</h3>
                            <p className="text-darkGray">Defensa</p>
                        </div>
                    </div>
                    {isDisabled ? (
                        <button
                            className="text-white mt-3 flex items-center justify-center"
                            onClick={handleClick}
                        >
                            Nuevo Pokemon
                        </button>
                    ) : (
                        <svg
                            aria-hidden="true"
                            className="mt-3 w-5 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                            viewBox="0 0 100 101"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                                fill="currentColor"
                            />
                            <path
                                d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                                fill="currentFill"
                            />
                        </svg>
                    )}
                </article>
            )}
        </>
    );
}
